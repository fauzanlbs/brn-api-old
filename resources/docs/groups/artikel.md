...
```bash
curl -X GET \
    -G "http://api.brn.com/api/articles/categories/odio?search=Berita+hari+ini&page[number]=1&page[size]=2&sort=created_at&filter[title]=Berita+hari+ini&filter[slug]=berita-hari-ini&filter[created_at]=2020-12-24&filter[featured]=1" \
    -H "Accept: application/json"
```

```javascript
const url = new URL(
    "http://api.brn.com/api/articles/categories/odio"
);
