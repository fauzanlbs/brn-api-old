...
class MemberController extends Controller
{
    ...
    public function getMembers(Request $request)
    {
        ...
        $users = QueryBuilder::for(User::class)->addSelect('regions.region', 'areas.area', 'subdistrict.subdistrict_name', 'users.*')
            ...
            ->when($status !== null, function($q, $search){
                ...
                return $q->join('addresses', 'users.id', '=', 'addresses.addressable_id')->join('regions', 'addresses.state', '=', 'regions.id')->join('areas', 'addresses.city', '=', 'areas.id')->join('subdistrict', 'addresses.street', '=', 'subdistrict.id');
            })
    ...
    public function memberDetail(User $user)
    {
        ...
        $user->select('regions.region', 'areas.area', 'subdistrict.subdistrict_name', 'users.*')->join('addresses', 'id', '=', 'addresses.addressable_id')->join('regions', 'addresses.state', '=', 'regions.id')->join('areas', 'addresses.city', '=', 'areas.id')->join('subdistrict', 'addresses.street', '=', 'subdistrict.id');
